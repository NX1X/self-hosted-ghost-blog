...
cleanup() {
    ...
    if [[ $exit_code -ne 0 && -f "$RECOVERY_SCRIPT" ]]; then
        ...
        echo ""
        echo "Need help? Check the migration logs above for error details." >&2
        echo "$SEPARATOR_SINGLE"
    fi
...
migrate_database() {
    ...
    if [[ $dump_status -ne 0 ]] || [[ "$dump_output" =~ "Error:" ]]; then
        ...
        if [[ "$dump_output" =~ "PROCESS privilege" ]]; then
            ...
            echo "Error details: $dump_output" >&2 >&2
        fi
        exit 1
    fi
    ...
    if [[ ! -f "$TEMP_SQL_FILE" ]] || [[ ! -s "$TEMP_SQL_FILE" ]]; then
        echo "" >&2
        echo "ERROR: Database dump file is empty or missing" >&2
    ...
    if [[ $counter -eq $MYSQL_TIMEOUT ]]; then
        echo ""
        echo "ERROR: Timed out waiting for MySQL container" >&2
        exit 1
    fi
...
find_ghost_installations() {
    ...
    if [[ -d "/var/www" ]]; then
        for dir in /var/www/*/; do
            ...
            if [[ -f "${dir}/.ghost-cli" ]] && [[ -d "${dir}/content" ]] && [[ -f "${dir}/config.production.json" ]]; then
                installations+=("$dir")
            fi
        done
    fi
...
main() {
    ...
    if [[ -z "$current_location" ]]; then
        echo "ERROR: Installation path is required" >&2
        exit 1
    fi

    # Validate Ghost installation
    if [[ ! -f "${current_location}/.ghost-cli" ]]; then
        echo "ERROR: No Ghost-CLI installation found at ${current_location}" >&2
        exit 1
    fi

    if [[ ! -d "${current_location}/content" ]]; then
        echo "ERROR: No content directory found at ${current_location}/content" >&2
        exit 1
    fi

    # Check for .env file
    if [[ ! -f "${PWD}/.env" ]]; then
        echo "ERROR: Please create a .env file for the Docker installation first" >&2
        exit 1
    fi
    ...
    if test_mysql_dump "$mysql_host" "$mysql_database" "$ghost_mysql_user" "$ghost_mysql_password"; then
        ...
        if ! test_mysql_dump "$mysql_host" "$mysql_database" "$mysql_user" "$mysql_password"; then
            echo ""
            echo "ERROR: The provided user doesn't have sufficient privileges for database export." >&2
    ...
    if ! systemctl stop "$ghost_service_name"; then
        echo "ERROR: Failed to stop Ghost service" >&2
    ...
    echo "  • For 502 errors, Ghost may still be starting (check logs)" >&2
